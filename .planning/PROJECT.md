...
### Active (v2.0 Launch Readiness)

- **Auth Foundation** — Create Supabase accounts, wire AuthContext, gate localStorage behind DEV — new
- **Auth Hardening** — Separate owner/employee login flows, remove demo shortcuts — new
- **Route Protection** — ProtectedRoute wrapper for all 30+ dashboard routes — new
- **Security Lockdown** — QR secret server-side, ALLOWED_ORIGINS, VIP RLS fix, reject unsigned QR — new
- **Scanner Improvements** — Auto-detect tonight's event, offline reject unknown, log failed scans, rate limit — new
- **Dashboard Accuracy** — Real revenue trends, fix ticket_count/type, staff names, targeted real-time — new
- **Bloat Cleanup** — Remove 5 monitoring pages, simplify sidebar, reduce analytics tabs — new
- **VIP & Events** — Floor plan drag-drop, invite sharing, table sync, remove hardcoded events, SEO — new
- **Code Quality** — Split orders-service.ts and AuthContext.tsx, organize components, strict TS — new
- **CI/CD & Deploy** — GitHub Actions secrets, Stripe production, Vercel env vars — new
