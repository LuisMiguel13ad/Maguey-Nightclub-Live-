...
must_haves:
  truths:
    - "Venue operator can diagnose common issues without technical knowledge"
    - "Each issue has clear symptom, diagnosis, resolution, and escalation path"
    - "Runbook covers payment failures, email issues, and scanner problems"
    - "Language is non-technical and actionable for event night staff"
  artifacts:
    - path: ".planning/SUPPORT-RUNBOOK.md"
      provides: "Symptom-based support documentation"
      min_lines: 300
  key_links:
    - from: "SUPPORT-RUNBOOK.md"
      to: "OwnerDashboard.tsx"
      via: "references dashboard locations for resolution"
      pattern: "Dashboard|Settings|Email Status"
---

<objective>
Support Runbook Creation

Purpose: Create a comprehensive support runbook for venue operators (non-technical staff) to diagnose and resolve common issues during events. Organized by symptom (what the customer/staff sees) with clear resolution steps.

Output: A single markdown file at .planning/SUPPORT-RUNBOOK.md that serves as the primary support documentation.
</objective>
...
# Error handling infrastructure (for accurate resolution steps)
...
# Dashboard for resolution steps
...
<task type="auto">
  <name>Task 1: Create support runbook structure and payment issues</name>
  <files>.planning/SUPPORT-RUNBOOK.md</files>
  <action>
Create the support runbook with payment failure issues:
...
```
  ...
  <verify>
File exists at .planning/SUPPORT-RUNBOOK.md and contains Payment Issues section with at least 4 issue entries
  </verify>
  <done>
Runbook created with structure and payment issues section (P-01 through P-04)
  </done>
</task>

<task type="auto">
  <name>Task 2: Add email issues section to runbook</name>
  <files>.planning/SUPPORT-RUNBOOK.md</files>
  <action>
Add email issues section to the runbook:
...
```
  ...
  <verify>
.planning/SUPPORT-RUNBOOK.md contains Email Issues section with E-01 through E-04
  </verify>
  <done>
Email issues section added with diagnosis and resolution for common email problems
  </done>
</task>

<task type="auto">
  <name>Task 3: Add scanner issues and appendix to runbook</name>
  <files>.planning/SUPPORT-RUNBOOK.md</files>
  <action>
Add scanner issues and appendix sections:
...
*Last updated: [Phase 11 completion date]*
*Version: 1.0*
```
  </action>
  <verify>
.planning/SUPPORT-RUNBOOK.md contains Scanner Issues (S-01 through S-05) and Appendix sections
  </verify>
  <done>
Complete runbook with payment issues (4), email issues (4), scanner issues (5), and appendix with navigation, escalation, error decoder, and emergency procedures
  </done>
</task>
...
<verification>
Runbook is complete and well-organized:
```bash
wc -l .planning/SUPPORT-RUNBOOK.md  # Should be 300+ lines
grep -c "^### " .planning/SUPPORT-RUNBOOK.md  # Should show 13+ issue entries
```
</verification>

<success_criteria>
1. Runbook exists at .planning/SUPPORT-RUNBOOK.md
2. Contains 300+ lines of content
3. Covers payment issues (P-01 through P-04)
4. Covers email issues (E-01 through E-04)
5. Covers scanner issues (S-01 through S-05)
6. Includes appendix with dashboard navigation, escalation contacts, error decoder
7. All language is non-technical and actionable for venue staff
8. Each issue has: Symptom, Diagnosis, Resolution, Escalation (where needed)
</success_criteria>

<output>
After completion, create `.planning/phases/11-error-handling-recovery/11-04-SUMMARY.md`
</output>
