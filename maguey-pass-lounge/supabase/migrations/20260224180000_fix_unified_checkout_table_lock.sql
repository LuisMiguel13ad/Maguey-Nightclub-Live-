-- Fix unified checkout premature table locking
-- create_unified_vip_checkout previously set is_available = false immediately.
-- This caused tables to be permanently unavailable if the user abandoned payment.
-- Availability should be dynamically computed via active/pending reservations.
...
BEGIN
  ...
  -- Create VIP reservation (Status is 'pending'. It functionally blocks checkout for others as long as it's active)
  ...
  -- Removed: UPDATE event_vip_tables SET is_available = false ...
  -- Table availability is now dynamically driven by the reservations table
