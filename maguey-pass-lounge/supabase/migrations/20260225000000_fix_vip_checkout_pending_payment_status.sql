-- Fix create_unified_vip_checkout to use pending_payment status
-- Previously, orders were created as 'paid' and tickets as 'valid' BEFORE payment completed.
-- Now they start as 'pending_payment' and get updated to 'paid'/'valid' by confirm-vip-payment.
...
BEGIN
  ...
  -- Create parent order with pending_payment status (NOT 'paid')
  ...
  -- Create GA ticket with pending_payment status (NOT 'valid')
  ...
  VALUES (v_order_id, p_event_id::uuid, p_tier_id, p_purchaser_email, p_purchaser_name, p_tier_price_cents / 100.0, v_unified_token, v_ticket_token, NOW(), 'pending_payment')
  RETURNING id INTO v_ticket_id;

  -- Create VIP reservation (pending until payment confirms)
