...
serve(async (req) => {
  ...
  try {
    ...
    const { paymentIntentId, reservationId, customerEmail } = await req.json();

    console.log("Confirming VIP payment:", { paymentIntentId, reservationId, customerEmail });

    // Validate required fields (including customerEmail for ownership verification)
