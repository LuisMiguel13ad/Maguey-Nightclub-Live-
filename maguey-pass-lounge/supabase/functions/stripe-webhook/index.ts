...
const getAllowedOrigin = (requestOrigin: string | null): string => {
  ...
  // Return first allowed origin as default (or * if list is empty)
...
// Helper to get CORS headers with proper origin for a request
...
serve(async (req) => {
  // Use dynamic CORS headers based on request origin
            ...
            if (vipInfo) {
              ...
              if (reservationError) {
                ...
                for (let i = 1; i <= guestCount; i++) {
                  ...
                  // Format: SHA-256(token|reservation_id|guest_number) in hex
                  // This matches the generate_vip_pass_signature() function in the database
