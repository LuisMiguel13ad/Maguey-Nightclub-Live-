...
const SendEmailStep: SagaStep<OrderSagaContext> = {
  ...
  async execute(context: OrderSagaContext): Promise<OrderSagaContext> {
    ...
    // Email sending is handled server-side via the email_queue table
    // and the process-email-queue Edge Function. The Stripe webhook
    // inserts emails into the queue after successful payment.
    // Client-side direct Resend API calls removed for security.
    stepLogger.info('Email delivery delegated to server-side email queue');
